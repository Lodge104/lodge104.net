...
locals {
  ...
  effective_project_name = var.use_parameter_store ? data.aws_ssm_parameter.project_name.value : var.project_name
  effective_region       = var.use_parameter_store ? data.aws_ssm_parameter.region.value : var.region
  ...
  effective_vpc_cidr     = var.use_parameter_store ? data.aws_ssm_parameter.vpc_cidr.value : var.vpc_cidr
  effective_db_password  = var.use_parameter_store ? data.aws_ssm_parameter.db_password.value : var.db_password
  ...
  effective_cf_price     = var.use_parameter_store ? data.aws_ssm_parameter.cloudfront_price_class.value : var.cloudfront_price_class
  effective_cf_ttl       = var.use_parameter_store ? tonumber(data.aws_ssm_parameter.cloudfront_default_ttl.value) : var.cloudfront_default_ttl
  effective_instance_type = var.use_parameter_store ? data.aws_ssm_parameter.instance_type.value : var.instance_type
  effective_capacity     = var.use_parameter_store ? tonumber(data.aws_ssm_parameter.desired_capacity.value) : var.desired_capacity
}
