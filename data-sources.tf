...
locals {
  ...
  effective_project_name = var.use_parameter_store && length(data.aws_ssm_parameter.project_name) > 0 ? data.aws_ssm_parameter.project_name[0].value : var.project_name
  effective_region       = var.use_parameter_store && length(data.aws_ssm_parameter.region) > 0 ? data.aws_ssm_parameter.region[0].value : var.region
  ...
  effective_vpc_cidr     = var.use_parameter_store && length(data.aws_ssm_parameter.vpc_cidr) > 0 ? data.aws_ssm_parameter.vpc_cidr[0].value : var.vpc_cidr
  effective_db_password  = var.use_parameter_store && length(data.aws_ssm_parameter.db_password) > 0 ? data.aws_ssm_parameter.db_password[0].value : var.db_password
  ...
  effective_cf_price     = var.use_parameter_store && length(data.aws_ssm_parameter.cloudfront_price_class) > 0 ? data.aws_ssm_parameter.cloudfront_price_class[0].value : var.cloudfront_price_class
  effective_cf_ttl       = var.use_parameter_store && length(data.aws_ssm_parameter.cloudfront_default_ttl) > 0 ? tonumber(data.aws_ssm_parameter.cloudfront_default_ttl[0].value) : var.cloudfront_default_ttl
  effective_instance_type = var.use_parameter_store && length(data.aws_ssm_parameter.instance_type) > 0 ? data.aws_ssm_parameter.instance_type[0].value : var.instance_type
  effective_capacity     = var.use_parameter_store && length(data.aws_ssm_parameter.desired_capacity) > 0 ? tonumber(data.aws_ssm_parameter.desired_capacity[0].value) : var.desired_capacity
}
